...
const PurchaseLicense: React.FC<PurchaseLicenseProps> = ({organizationId }) => {
  ...
  const handleLicenseCountChange = (value: number) => {
    ...
    const matchingLicense = licensePackages.find(pkg => {
      const maxLicense = typeof pkg.maximum_license === 'string' ? Number(pkg.maximum_license) : pkg.maximum_license;
    ...
    if (matchingLicense) {
      setSelectedLicense({
        ...
        maximum_license: Number(matchingLicense.maximum_license), // Convert to number
          ...
          <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-lg">
            ...
            <div className="flex justify-end gap-4">
              ...
              <button
                ...
                className={`bg-blue-500 text-white p-2 rounded-lg mt-4 w-full ${loadingPackageId === totalAmount ? 'opacity-50 cursor-not-allowed' : ''}`}
