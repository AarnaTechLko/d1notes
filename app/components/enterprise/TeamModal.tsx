  ...
  const CustomOption = (props: any) => {
    ...
    return (
      <div ref={innerRef} {...innerProps} className="p-4">
        ...
        <div>{/* Add any other information you want */}</div>
      </div>
    );
  };
    ...
    const fetchCoaches = async () => {
      try {
        ...
        const response = await fetch(
          `/api/enterprise/coach?enterprise_id=${enterprise_id}`
        );
        ...
        if (team?.coach_id) {
          const matchedCoach = formattedCoaches.find(
            (coach: any) =>
              coach.value === team?.coach_id?.toString() || undefined
          );
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
      ...
      >
        <h2 className="text-xl font-bold mb-4">
          {team ? "Edit Team Manually" : "Add Team Manually"}
        </h2>
        <form
