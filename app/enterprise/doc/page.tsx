...
const Home: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    if (!phone) {
      ...
      if (!firstError)
        firstError = "Phone number must be in the format (XXX) XXX-XXXX.";
      isValid = false;
    }
  ...
  const fetchOrders = async () => {
    ...
    const response = await fetch(
      `/api/enterprise/doc?club_id=${enterpriseId}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  const totalPages =
    filteredOrders.length === 0 ? 1 : Math.ceil(filteredOrders.length / limit);
  ...
  const handleResetpassword = async (record: Order) => {
    // Add 'async' here
    Swal.fire({
      title: "Are you sure?",
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            ...
                            onChange={() =>
                              setAcceptEvaluations(!acceptEvaluations)
                            }
                          />
                          <div className="w-14 h-7 bg-gray-300 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer dark:bg-gray-600 peer-checked:after:translate-x-7 after:content-[''] after:absolute after:top-1 after:left-1 after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-500"></div>
                        </label>
                ...
                {paginatedOrders.length > 0 ? (
                  paginatedOrders.map((order, index) => (
                    <tr key={order.id}>
                      ...
                      <td>{order.email}</td>
                      <td>
                        {order.countryCodes} {order.phone}
                      </td>
                      <td>
                        {order.buy_evaluation === "true" && (
                          <p>Purchase Evaluations</p>
                        )}
                        {order.view_evaluation === "true" && (
                          <p>View Evaluations</p>
                        )}
                      </td>
                      <td>
                        {/* <button
